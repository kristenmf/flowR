...
#dunn.test(L[, 7], factor(stim_type), kw = TRUE, label = TRUE)
...
## Correlation between double positive frequencies
...
o <- c(7, 14, 8, 6, 11, 5, 1, 2, 10, 13, 4, 9, 3, 12)
# par(mfrow = c(1, 1))
# plot(1:14, betw$c1[, 3][c(7, 14, 8, 6, 11, 5, 1, 2, 10, 13, 4, 9, 3, 12)], type = 'n')
# text(1:14, betw$c1[, 3][c(7, 14, 8, 6, 11, 5, 1, 2, 10, 13, 4, 9, 3, 12)], labels = colnames(L2)[c(7, 14, 8, 6, 11, 5, 1, 2, 10, 13, 4, 9, 3, 12)], cex = 0.5, col = c(1, 1, 1, 2, 2, 2, 2, 3, 3, 4, 4, 4, 4, 4))

pheatmap(cor(L2)[o, o], cluster_rows = FALSE, cluster_cols = FALSE)
```

These four groups of double positive combinations should be fairly homogenous and therefore very well described by a single principal component - i.e. it should be possible to give the stimulations a straightforward ordering to describe the relative frequencies. 
...
pca <- dudi.pca(L2[, c(3, 13, 12, 4, 9)], scan = FALSE, nf = 8)
betw <- bca(pca, factor(stim[sample_fac$stim]), scan = FALSE, nf = 3)
par(mfrow = c(1, 2))
s.class(betw$ls, factor(stim[sample_fac$stim]), col = sample_fac$stim)
s.arrow(betw$c1, clabel = 0.5)
...
* Influenza has highest frequency. birchgrass has lowest frequency. 

```{r,echo=FALSE}
pca <- dudi.pca(L2[, c(2, 10)], scan = FALSE, nf = 8)
par(mfrow = c(1, 2))
s.class(pca$li, factor(stim[sample_fac$stim]), col = sample_fac$stim)
s.arrow(pca$c1, clabel = 0.5)
...
Also make boxplots and kruskal-wallis test for the three stimulation types gut, lung and virus. 

```{r,echo=FALSE}

par(mfrow = c(3, 5))
for (i in 1:14){
  pval <- signif(kruskal.test(L2[, i], factor(stim_type))$p.value, 2)
  boxplot(L2[, i] ~ factor(stim_type), main = paste(colnames(L2)[i], pval), las = 2)
          }
...
* __group 4__ (IFNg-GMCSF, GMCSF-IL10)
+ correlated to GMCSF
+ anticorrelated to unstained
+  IFNg-GMCSF double positive frequency strongly associated with GMCSF bulk frequency
...
```{r,echo=FALSE}
# It is of interest to know the connection between double positive frequencies and bulk frequencies. To do this, calculate the cross-correlation matrix, e.g. between IFNg/IL17 double positive frequencies and IL10 bulk frequencies, and so on. The group structure of above is maintained, except interestingly, IFNg-GMSCF switches to group 1 (I don't know why yet). Then we can see:
# 
...
pheatmap(C[, o], cluster_cols = FALSE)
...
```{r,echo=FALSE}
pheatmap(cor(C)[o, o], cluster_rows = FALSE, cluster_cols = FALSE)
```
...
Kruskal-wallis tests show that all markers have at least one stochastically dominant stimulation subtype for bulk frequencies. This suggests that bulk frequencies alone are a very good fingerprint of immune response. 

Interestingly, there isn't such a strong difference between stimulation subtypes for double positive frequencies. (and virtually none for contingency table expected values). This is because double positive frequencies are not independent of the bulk frequencies. For example, 5% double positive frequency in two different samples can mean different underlying biological processes depending on whether the bulk frequencies are 10% or 20%. 
